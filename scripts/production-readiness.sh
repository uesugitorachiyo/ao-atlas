...
jq -e '.completed_recommendation_nodes == 1 and .lease_health_status == "minimum_unmet" and .generated_workgraph.ready_nodes == 39 and .generated_workgraph.executable_ready_nodes == 1 and .generated_workgraph.first_executable_node == "mission-recommendation-next-02" and .generated_workgraph.lease_health_status == .lease_health_status and .generated_workgraph.checkpoint_count == 1 and .generated_workgraph.return_gate_status == "blocked_ready_nodes_remain" and .generated_workgraph.final_response_allowed == false and .foundry_run_link_readiness_summary.completed_run_links == 1 and .foundry_run_link_readiness_summary.required_run_links == 40 and .foundry_run_link_readiness_summary.next_executable_node == "mission-recommendation-next-02" and .foundry_run_link_readiness_summary.lease_health_status == .lease_health_status and (.source_artifacts[] | select(.ref == "foundry_run_link_readiness_summary" and (.digest | test("^sha256:[0-9a-f]{64}$"))))' "$OUT/mission-recommendations/execution-readback-after-node-01.json" >/dev/null
...
jq -e '.lease_health_status == "minimum_unmet" and .generated_workgraph.lease_health_status == .lease_health_status and .foundry_run_link_readiness_summary.completed_run_links == 1 and .foundry_run_link_readiness_summary.required_run_links == 40 and .foundry_run_link_readiness_summary.next_executable_node == "mission-recommendation-next-02" and .foundry_run_link_readiness_summary.lease_health_status == .lease_health_status and (.source_artifacts[] | select(.ref == "foundry_run_link_readiness_summary" and (.digest | test("^sha256:[0-9a-f]{64}$"))))' "$OUT/mission-recommendations/execution-readback-resumed.json" >/dev/null
jq -e '.schema == "ao.atlas.recommendation-command-readback.v0.1" and .elapsed_minutes == 25 and .lease_time_status == "minimum_minutes_unmet" and .lease_health_status == "minimum_unmet" and .checkpoint_count == 1 and .return_gate_status == "blocked_ready_nodes_remain" and .final_response_allowed == false and .claims_authority_advance == false and .command_timeline_binding.summary == .compact_timeline and .command_timeline_binding.exact_next_action == .exact_next_action and .command_timeline_binding.return_gate_status == .return_gate_status and .command_timeline_binding.lease_health_status == .lease_health_status' "$OUT/mission-recommendations/command-readback-resumed.json" >/dev/null
jq -e '.schema == "ao.atlas.recommendation-promoter-readback.v0.1" and .lease_health_status == "minimum_unmet" and .promotion_claimed == false and .rsi_remains_denied == true and .claims_authority_advance == false' "$OUT/mission-recommendations/promoter-readback-resumed.json" >/dev/null
...
jq -e '.schema == "ao.atlas.recommendation-foundry-rollup.v0.1" and .node_completion_status == "nodes_in_progress" and .lease_completion_status == "minimum_minutes_unmet" and .lease_health_status == "minimum_unmet" and .checkpoint_count == 1 and .return_gate_status == "blocked_ready_nodes_remain" and .final_response_allowed == false and .claims_authority_advance == false' "$OUT/mission-recommendations/foundry-rollup-resumed.json" >/dev/null
jq -e '.schema == "ao.atlas.recommendation-reconciliation-packet.v0.1" and .status == "continuation_required" and .checkpoint_count == 1 and .return_gate_status == "blocked_ready_nodes_remain" and .lease_health_status == "minimum_unmet" and .command_return_gate_status == "blocked_ready_nodes_remain" and .foundry_return_gate_status == "blocked_ready_nodes_remain" and .artifacts_agree == true and .promotion_claimed == false and .rsi_remains_denied == true and .claims_authority_advance == false' "$OUT/mission-recommendations/reconciliation-packet-resumed.json" >/dev/null
