...
jq -e '.completed_recommendation_nodes == 1 and .generated_workgraph.ready_nodes == 39 and .generated_workgraph.executable_ready_nodes == 1 and .generated_workgraph.first_executable_node == "mission-recommendation-next-02" and .generated_workgraph.checkpoint_count == 1 and .generated_workgraph.return_gate_status == "blocked_ready_nodes_remain" and .generated_workgraph.final_response_allowed == false and .foundry_run_link_readiness_summary.completed_run_links == 1 and .foundry_run_link_readiness_summary.required_run_links == 40 and .foundry_run_link_readiness_summary.next_executable_node == "mission-recommendation-next-02" and (.source_artifacts[] | select(.ref == "foundry_run_link_readiness_summary" and (.digest | test("^sha256:[0-9a-f]{64}$"))))' "$OUT/mission-recommendations/execution-readback-after-node-01.json" >/dev/null
