...
grep -q "Command readback requirements stay compact and gate-bound" docs/sdd/AO-ATLAS-LONG-RUN-RECOMMENDATIONS.md
...
"$BIN" mission recommendations readback --wave "$OUT/mission-recommendations/recommendation-wave.json" --workgraph "$OUT/mission-recommendations/recommendation-workgraph.json" --evidence-root target/production-readiness/mission-recommendations --out "$OUT/mission-recommendations/recommendation-readback-regenerated.json" --out-execution-readback "$OUT/mission-recommendations/execution-readback-regenerated.json" --out-workgraph-readiness-packet "$OUT/mission-recommendations/workgraph-readiness-packet-regenerated.json" >/dev/null
test -s "$OUT/mission-recommendations/recommendation-readback-regenerated.json"
test -s "$OUT/mission-recommendations/execution-readback-regenerated.json"
test -s "$OUT/mission-recommendations/workgraph-readiness-packet-regenerated.json"
jq -e '.node_budget == 40 and .continue_if_fast_target == 40 and .return_gate_status == "blocked_ready_nodes_remain" and .continuation_contract_reason == "ready_nodes_or_exact_next_action_remain" and .final_response_allowed == false' "$OUT/mission-recommendations/workgraph-readiness-packet-regenerated.json" >/dev/null
jq -e '.continuation_contract_reason == "ready_nodes_or_exact_next_action_remain" and .reason_artifact_agreement_summary.status == "agreement" and .reason_artifact_agreement_summary.expected_reason == .continuation_contract_reason and .reason_artifact_agreement_summary.source_count == .continuation_reason_coverage.source_count and .reason_artifact_agreement_summary.all_required_sources_indexed == true and .reason_artifact_agreement_summary.source_artifact_count == (.source_artifacts | length) and .reason_artifact_agreement_summary.source_artifacts_agree == true and .reason_artifact_agreement_summary.final_response_allowed == .final_response_allowed and .reason_artifact_agreement_summary.refuses_final_response == .refuses_final_response and .reason_artifact_agreement_summary.claims_authority_advance == false and .reason_artifact_agreement_summary.rsi_remains_denied == true' "$OUT/mission-recommendations/execution-readback-regenerated.json" >/dev/null
...
jq -e '.completed_recommendation_nodes == 1 and .lease_health_status == "minimum_unmet" and .checkpoint_freshness_status == "fresh_checkpoint_required_after_each_node_or_timed_interval" and .return_gate_status == "blocked_ready_nodes_remain" and .continuation_contract_reason == "ready_nodes_or_exact_next_action_remain" and .exact_next_action == "Emit Foundry import for mission-recommendation-next-02 and execute exactly one active node." and .final_response_allowed == false and .refuses_final_response == true and .generated_workgraph.ready_nodes == 39 and .generated_workgraph.executable_ready_nodes == 1 and .generated_workgraph.first_executable_node == "mission-recommendation-next-02" and .generated_workgraph.lease_health_status == .lease_health_status and .generated_workgraph.checkpoint_freshness_status == .checkpoint_freshness_status and .generated_workgraph.checkpoint_count == 1 and .generated_workgraph.return_gate_status == "blocked_ready_nodes_remain" and .generated_workgraph.final_response_allowed == false and .foundry_run_link_readiness_summary.completed_run_links == 1 and .foundry_run_link_readiness_summary.required_run_links == 40 and .foundry_run_link_readiness_summary.next_executable_node == "mission-recommendation-next-02" and .foundry_run_link_readiness_summary.lease_health_status == .lease_health_status and .foundry_run_link_readiness_summary.checkpoint_freshness_status == .checkpoint_freshness_status and .foundry_run_link_readiness_summary.return_gate_status == .return_gate_status and .foundry_run_link_readiness_summary.continuation_contract_reason == .continuation_contract_reason and .foundry_run_link_readiness_summary.exact_next_action == .exact_next_action and .foundry_run_link_readiness_summary.refuses_final_response == .refuses_final_response and .continuation_reason_coverage.status == "coverage_sources_indexed" and .continuation_reason_coverage.expected_reason == "ready_nodes_or_exact_next_action_remain" and .continuation_reason_coverage.source_count == 13 and (.continuation_reason_coverage.indexed_sources | index("checkpoint_readback") != null) and (.continuation_reason_coverage.indexed_sources | index("resume_prompt") != null) and .continuation_reason_coverage.refuses_final_response == true and .continuation_reason_coverage.final_response_allowed == false and .reason_artifact_agreement_summary.status == "agreement" and .reason_artifact_agreement_summary.expected_reason == .continuation_contract_reason and .reason_artifact_agreement_summary.source_count == .continuation_reason_coverage.source_count and .reason_artifact_agreement_summary.all_required_sources_indexed == true and .reason_artifact_agreement_summary.source_artifact_count == (.source_artifacts | length) and (.reason_artifact_agreement_summary.source_artifact_refs | index("continuation_reason_coverage") != null) and (.reason_artifact_agreement_summary.source_artifact_refs | index("foundry_run_link_readiness_summary") != null) and .reason_artifact_agreement_summary.source_artifacts_agree == true and .reason_artifact_agreement_summary.final_response_allowed == .final_response_allowed and .reason_artifact_agreement_summary.refuses_final_response == .refuses_final_response and .reason_artifact_agreement_summary.claims_authority_advance == false and .reason_artifact_agreement_summary.rsi_remains_denied == true and (.source_artifacts[] | select(.ref == "foundry_run_link_readiness_summary" and (.digest | test("^sha256:[0-9a-f]{64}$")))) and (.source_artifacts[] | select(.ref == "continuation_reason_coverage" and (.digest | test("^sha256:[0-9a-f]{64}$"))))' "$OUT/mission-recommendations/execution-readback-after-node-01.json" >/dev/null
jq -e '.schema == "ao.atlas.recommendation-checkpoint-readback.v0.1" and .completed_nodes == 1 and .ready_nodes == 39 and .elapsed_minutes == 17 and .min_minutes_met == false and .lease_health_status == "minimum_unmet" and .continuation_contract_reason == "ready_nodes_or_exact_next_action_remain" and .final_response_allowed == false' "$OUT/mission-recommendations/checkpoint-readback-after-node-01.json" >/dev/null
...
jq -e '.lease_health_status == "minimum_unmet" and .checkpoint_freshness_status == "fresh_checkpoint_required_after_each_node_or_timed_interval" and .return_gate_status == "blocked_ready_nodes_remain" and .continuation_contract_reason == "ready_nodes_or_exact_next_action_remain" and .exact_next_action == "Emit Foundry import for mission-recommendation-next-02 and execute exactly one active node." and .final_response_allowed == false and .refuses_final_response == true and .generated_workgraph.lease_health_status == .lease_health_status and .generated_workgraph.checkpoint_freshness_status == .checkpoint_freshness_status and .foundry_run_link_readiness_summary.completed_run_links == 1 and .foundry_run_link_readiness_summary.required_run_links == 40 and .foundry_run_link_readiness_summary.next_executable_node == "mission-recommendation-next-02" and .foundry_run_link_readiness_summary.lease_health_status == .lease_health_status and .foundry_run_link_readiness_summary.checkpoint_freshness_status == .checkpoint_freshness_status and .foundry_run_link_readiness_summary.return_gate_status == .return_gate_status and .foundry_run_link_readiness_summary.continuation_contract_reason == .continuation_contract_reason and .foundry_run_link_readiness_summary.exact_next_action == .exact_next_action and .foundry_run_link_readiness_summary.refuses_final_response == .refuses_final_response and .continuation_reason_coverage.status == "coverage_sources_indexed" and .continuation_reason_coverage.expected_reason == "ready_nodes_or_exact_next_action_remain" and .continuation_reason_coverage.source_count == 13 and (.continuation_reason_coverage.indexed_sources | index("checkpoint_readback") != null) and (.continuation_reason_coverage.indexed_sources | index("resume_prompt") != null) and .continuation_reason_coverage.refuses_final_response == true and .continuation_reason_coverage.final_response_allowed == false and .reason_artifact_agreement_summary.status == "agreement" and .reason_artifact_agreement_summary.expected_reason == .continuation_contract_reason and .reason_artifact_agreement_summary.source_count == .continuation_reason_coverage.source_count and .reason_artifact_agreement_summary.all_required_sources_indexed == true and .reason_artifact_agreement_summary.source_artifact_count == (.source_artifacts | length) and (.reason_artifact_agreement_summary.source_artifact_refs | index("continuation_reason_coverage") != null) and (.reason_artifact_agreement_summary.source_artifact_refs | index("foundry_run_link_readiness_summary") != null) and .reason_artifact_agreement_summary.source_artifacts_agree == true and .reason_artifact_agreement_summary.final_response_allowed == .final_response_allowed and .reason_artifact_agreement_summary.refuses_final_response == .refuses_final_response and .reason_artifact_agreement_summary.claims_authority_advance == false and .reason_artifact_agreement_summary.rsi_remains_denied == true and (.source_artifacts[] | select(.ref == "foundry_run_link_readiness_summary" and (.digest | test("^sha256:[0-9a-f]{64}$")))) and (.source_artifacts[] | select(.ref == "continuation_reason_coverage" and (.digest | test("^sha256:[0-9a-f]{64}$"))))' "$OUT/mission-recommendations/execution-readback-resumed.json" >/dev/null
jq -e '. as $cmd | .schema == "ao.atlas.recommendation-command-readback.v0.1" and .elapsed_minutes == 25 and .lease_time_status == "minimum_minutes_unmet" and .lease_health_status == "minimum_unmet" and .checkpoint_freshness_status == "fresh_checkpoint_required_after_each_node_or_timed_interval" and .checkpoint_count == 1 and .return_gate_status == "blocked_ready_nodes_remain" and .continuation_contract_reason == "ready_nodes_or_exact_next_action_remain" and .final_response_allowed == false and .claims_authority_advance == false and ($cmd.compact_timeline | contains("continuation_contract_reason=" + $cmd.continuation_contract_reason)) and ($cmd.compact_timeline | contains("exact_next_action=" + $cmd.exact_next_action)) and .command_timeline_binding.summary == .compact_timeline and .command_timeline_binding.exact_next_action == .exact_next_action and .command_timeline_binding.continuation_contract_reason == .continuation_contract_reason and .command_timeline_binding.return_gate_status == .return_gate_status and .command_timeline_binding.lease_health_status == .lease_health_status and .command_timeline_binding.checkpoint_freshness_status == .checkpoint_freshness_status' "$OUT/mission-recommendations/command-readback-resumed.json" >/dev/null
jq -e '. as $promoter | .schema == "ao.atlas.recommendation-promoter-readback.v0.1" and .lease_health_status == "minimum_unmet" and .checkpoint_freshness_status == "fresh_checkpoint_required_after_each_node_or_timed_interval" and .continuation_contract_reason == "ready_nodes_or_exact_next_action_remain" and ($promoter.no_promotion_reason_summary | contains("continuation_contract_reason=" + $promoter.continuation_contract_reason)) and ($promoter.no_promotion_reason_summary | contains("final_response_allowed=false")) and .promotion_claimed == false and .rsi_remains_denied == true and .claims_authority_advance == false' "$OUT/mission-recommendations/promoter-readback-resumed.json" >/dev/null
...
jq -e '.schema == "ao.atlas.recommendation-foundry-rollup.v0.1" and .node_completion_status == "nodes_in_progress" and .lease_completion_status == "minimum_minutes_unmet" and .lease_health_status == "minimum_unmet" and .checkpoint_freshness_status == "fresh_checkpoint_required_after_each_node_or_timed_interval" and .checkpoint_count == 1 and .return_gate_status == "blocked_ready_nodes_remain" and .continuation_contract_reason == "ready_nodes_or_exact_next_action_remain" and .final_response_allowed == false and .claims_authority_advance == false' "$OUT/mission-recommendations/foundry-rollup-resumed.json" >/dev/null
jq -e '.schema == "ao.atlas.recommendation-reconciliation-packet.v0.1" and .status == "continuation_required" and .checkpoint_count == 1 and .return_gate_status == "blocked_ready_nodes_remain" and .lease_health_status == "minimum_unmet" and .checkpoint_freshness_status == "fresh_checkpoint_required_after_each_node_or_timed_interval" and .stale_route_decision_status == "fresh_atlas_supervises_foundry_owns_one_active_node" and .continuation_contract_reason == "ready_nodes_or_exact_next_action_remain" and .command_continuation_contract_reason == .continuation_contract_reason and .promoter_continuation_contract_reason == .continuation_contract_reason and .foundry_continuation_contract_reason == .continuation_contract_reason and .continuation_reason_agreement == true and .command_return_gate_status == "blocked_ready_nodes_remain" and .foundry_return_gate_status == "blocked_ready_nodes_remain" and .artifacts_agree == true and .promotion_claimed == false and .rsi_remains_denied == true and .claims_authority_advance == false and .final_state_reconciliation.contract_version == "ao.atlas.final-state-reconciliation.v0.1" and .final_state_reconciliation.status == .status and .final_state_reconciliation.workgraph_status == "in_progress" and .final_state_reconciliation.foundry_rollup_status == .foundry_status and .final_state_reconciliation.promoter_verdict_status == .promoter_status and .final_state_reconciliation.command_readback_status == "in_progress" and .final_state_reconciliation.exact_next_action == .exact_next_action and .final_state_reconciliation.continuation_contract_reason == .continuation_contract_reason and .final_state_reconciliation.continuation_reason_agreement == .continuation_reason_agreement and .final_state_reconciliation.schedules_work == false and .final_state_reconciliation.executes_work == false and .final_state_reconciliation.approves_work == false' "$OUT/mission-recommendations/reconciliation-packet-resumed.json" >/dev/null
jq -e '.schema == "ao.atlas.recommendation-reconciliation-packet.v0.1" and .status == "blocked_stale_artifact" and .artifacts_agree == false and .completed_nodes == 1 and .ready_nodes == 39 and .return_gate_status == "blocked_ready_nodes_remain" and .command_final_response_allowed == false and .final_response_allowed == false and .final_state_reconciliation.status == "blocked_stale_artifact" and .final_state_reconciliation.workgraph_status == "in_progress" and .final_state_reconciliation.command_readback_status == "completed" and .promotion_claimed == false and .rsi_remains_denied == true and .claims_authority_advance == false' examples/invalid/recommendation-reconciliation-stale-command-status.json >/dev/null
jq -e '.schema == "ao.atlas.recommendation-reconciliation-packet.v0.1" and .status == "blocked_stale_artifact" and .artifacts_agree == false and .completed_nodes == 1 and .ready_nodes == 39 and .return_gate_status == "blocked_ready_nodes_remain" and .continuation_contract_reason == "ready_nodes_or_exact_next_action_remain" and .command_continuation_contract_reason == "ready_nodes_remain" and .promoter_continuation_contract_reason == .continuation_contract_reason and .foundry_continuation_contract_reason == .continuation_contract_reason and .continuation_reason_agreement == false and .final_state_reconciliation.status == "blocked_stale_artifact" and .final_state_reconciliation.command_readback_status == "in_progress" and .final_state_reconciliation.continuation_contract_reason == .continuation_contract_reason and .final_state_reconciliation.continuation_reason_agreement == false and .promotion_claimed == false and .rsi_remains_denied == true and .claims_authority_advance == false' examples/invalid/recommendation-reconciliation-stale-continuation-reason.json >/dev/null
...
grep -q "Continuation contract reason: \`ready_nodes_or_exact_next_action_remain\`" "$OUT/mission-recommendations/next-recommended-prompt-resumed.md"
...
while IFS= read -r execution_readback; do
  ...
  if jq -e 'has("continuation_reason_coverage") and has("source_artifacts")' "$execution_readback" >/dev/null; then
    jq -e '
      .reason_artifact_agreement_summary.status == "agreement" and
      .reason_artifact_agreement_summary.expected_reason == .continuation_reason_coverage.expected_reason and
      .reason_artifact_agreement_summary.source_count == .continuation_reason_coverage.source_count and
      .reason_artifact_agreement_summary.all_required_sources_indexed == true and
      .reason_artifact_agreement_summary.source_artifact_count == (.source_artifacts | length) and
      (.reason_artifact_agreement_summary.source_artifact_refs | index("continuation_reason_coverage") != null) and
      (.reason_artifact_agreement_summary.source_artifact_refs | index("foundry_run_link_readiness_summary") != null) and
      .reason_artifact_agreement_summary.source_artifacts_agree == true and
      .reason_artifact_agreement_summary.final_response_allowed == .final_response_allowed and
      .reason_artifact_agreement_summary.refuses_final_response == .refuses_final_response and
      .reason_artifact_agreement_summary.claims_authority_advance == false and
      .reason_artifact_agreement_summary.rsi_remains_denied == true
    ' "$execution_readback" >/dev/null
  fi
...
lease_resume_command="$lease_resume_root/command-readback.json"
lease_resume_promoter="$lease_resume_root/promoter-readback.json"
...
if [ -f "$lease_resume_readback" ]; then
  ...
  test -s "$lease_resume_command"
  test -s "$lease_resume_promoter"
  ...
  jq -e --slurpfile readback "$lease_resume_readback" '
    ...
    .reason_artifact_agreement_summary.status == "agreement" and
    .reason_artifact_agreement_summary.expected_reason == .continuation_contract_reason and
    .reason_artifact_agreement_summary.source_count == .continuation_reason_coverage.source_count and
    .reason_artifact_agreement_summary.all_required_sources_indexed == true and
    .reason_artifact_agreement_summary.source_artifact_count == (.source_artifacts | length) and
    (.reason_artifact_agreement_summary.source_artifact_refs | index("continuation_reason_coverage") != null) and
    ...
    .reason_artifact_agreement_summary.refuses_final_response == .refuses_final_response and
    .reason_artifact_agreement_summary.claims_authority_advance == false and
    .reason_artifact_agreement_summary.rsi_remains_denied == true and
  ...
  jq -e --slurpfile readback "$lease_resume_readback" '
    ...
    .continuation_contract_reason == $readback[0].continuation_contract.reason and
    .exact_next_action == $readback[0].exact_next_action and
    ($cmd.compact_timeline | contains("continuation_contract_reason=" + $cmd.continuation_contract_reason)) and
    ($cmd.compact_timeline | contains("exact_next_action=" + $cmd.exact_next_action)) and
    .command_timeline_binding.summary == .compact_timeline and
    .command_timeline_binding.continuation_contract_reason == .continuation_contract_reason and
    .command_timeline_binding.exact_next_action == .exact_next_action
  ' "$lease_resume_command" >/dev/null
  jq -e --slurpfile readback "$lease_resume_readback" '
    . as $promoter |
    .continuation_contract_reason == $readback[0].continuation_contract.reason and
    .final_response_allowed == $readback[0].final_response_allowed and
    ($promoter.no_promotion_reason_summary | contains("continuation_contract_reason=" + $promoter.continuation_contract_reason)) and
    ($promoter.no_promotion_reason_summary | contains("final_response_allowed=true")) and
    .promotion_claimed == false and
    .rsi_remains_denied == true and
    .claims_authority_advance == false
  ' "$lease_resume_promoter" >/dev/null
