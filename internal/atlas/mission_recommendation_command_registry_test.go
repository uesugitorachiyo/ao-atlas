...
func TestMissionRecommendationCommandRegistryDrivesDeterministicDispatchHelp(t *testing.T) {
	...
	if !strings.Contains(text, "mission recommendations requires import, export-next-wave, export-refactoring-wave") ||
		!strings.Contains(text, "mission-dashboard-compact-filters, bounded-signer-contract-fixture, canonical-contract-registry-manifest, contract-compatibility-inventory, canonical-json-vectors, complete-node, resume, or validate-evidence") {
