...
func TestP0BPolicyVersionReplayRejectionFixtureRejectsStaleTickets(t *testing.T) {
	root := repoRoot(t)
	nodeDir := filepath.Join(root, "docs", "evidence", "ao-stack-p0b-contract-convergence-wave-v01", "nodes", "mission-recommendation-p0b-contract-convergence-09")
	...
	if !strings.Contains(out.String(), "status=stale_policy_versions_rejected") ||
		...
		t.Fatalf("policy version replay output missing summary: %s", out.String())
	}
	recorded := mustLoadJSON[map[string]any](t, recordedPath)
	generated := mustLoadJSON[map[string]any](t, outPath)
	if digestValue(generated) != digestValue(recorded) {
		t.Fatalf("policy version replay fixture changed\nwant %s\ngot  %s", digestValue(recorded), digestValue(generated))
	}
	if generated["safe_to_accept"] != false ||
		...
		t.Fatalf("policy version replay fixture lost safety state: %#v", generated)
	}
}

func TestP0BPolicyVersionReplayRejectionFixtureUsesTypedValidator(t *testing.T) {
	root := repoRoot(t)
	path := filepath.Join(root, "docs", "evidence", "ao-stack-p0b-contract-convergence-wave-v01", "nodes", "mission-recommendation-p0b-contract-convergence-09", "policy-version-replay-rejection-fixture.json")
