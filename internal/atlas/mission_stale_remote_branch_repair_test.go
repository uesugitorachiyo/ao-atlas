...
func TestFeatureDepthWaveStaleRemoteBranchRepairFixtureRecordsInterruptedCleanupHandoffs(t *testing.T) {
	root := repoRoot(t)
	nodeDir := filepath.Join(root, "docs", "evidence", "ao-atlas-feature-depth-wave-v01", "nodes", "mission-recommendation-feature-depth-next-wave-14")
...
func TestFeatureDepthWaveStaleRemoteBranchRepairFixtureUsesTypedValidator(t *testing.T) {
	root := repoRoot(t)
	path := filepath.Join(root, "docs", "evidence", "ao-atlas-feature-depth-wave-v01", "nodes", "mission-recommendation-feature-depth-next-wave-14", "stale-remote-branch-repair.json")
