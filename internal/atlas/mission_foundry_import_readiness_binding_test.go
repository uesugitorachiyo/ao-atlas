...
func TestFeatureDepthWaveV02FoundryImportReadinessBindingKeepsOneActiveNode(t *testing.T) {
	root := repoRoot(t)
	...
	generated := mustLoadJSON[AtlasFoundryImportReadinessBinding](t, outPath)
	if digestValue(generated) != digestValue(recorded) {
		t.Fatalf("v02 Foundry import readiness binding fixture changed\nwant %s\ngot  %s", digestValue(recorded), digestValue(generated))
	}
	if err := ValidateAtlasFoundryImportReadinessBinding(recorded); err != nil {
		t.Fatal(err)
	}
	if recorded.Status != "single_active_foundry_import_ready" ||
		...
		recorded.ClaimsAuthorityAdvance ||
		!recorded.RSIRemainsDenied {
		t.Fatalf("v02 Foundry import readiness binding lost single-active-node state: %#v", recorded)
	}
