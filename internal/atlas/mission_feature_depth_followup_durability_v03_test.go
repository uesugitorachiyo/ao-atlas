...
func TestFeatureDepthFollowupDurabilityV03CompletesFortyNodeLongRunWave(t *testing.T) {
	root := filepath.Join(repoRoot(t), "docs", "evidence", "ao-atlas-feature-depth-followup-durability-v03")
	...
	if wave.MissionID != "ao-atlas-next-feature-depth-followup-durability-v03" ||
		wave.TargetInstance != "ao-atlas-feature-depth-followup-durability-v03" ||
		wave.TotalTasks != 40 ||
		wave.MinimumTasks != 40 ||
		wave.NodeBudget != 40 ||
		wave.FinalResponseAllowed {
		t.Fatalf("v03 wave must preserve the 40-node Feature Depth contract: %#v", wave)
	}
	if wave.Supervisor == nil ||
		...
		t.Fatalf("v03 wave must preserve the long-run supervisor lease: %#v", wave.Supervisor)
	}
	...
	if readback.CompletedNodes != 40 ||
		...
		t.Fatalf("v03 final readback must close the long-run wave cleanly: %#v", readback)
	}
	...
	if report.Schema != AtlasRecommendationEvidenceValidationReportContract ||
		...
		t.Fatalf("v03 validation report must cover all node evidence without failures: %#v", report)
	}
	...
	if handoff.Schema != "ao.atlas.final-feature-depth-recommendations.v0.1" ||
		...
		t.Fatalf("v03 final handoff must preserve 40 no-promotion Feature Depth recommendations: %#v", handoff)
	}
