...
func TestFeatureDepthWaveV02MissionDashboardClosureBindingRowsBindAtlasClosureEvidence(t *testing.T) {
	root := repoRoot(t)
	...
	generated := mustLoadJSON[AtlasMissionDashboardClosureBinding](t, outPath)
	if digestValue(generated) != digestValue(recorded) {
		t.Fatalf("v02 Mission dashboard closure binding fixture changed\nwant %s\ngot  %s", digestValue(recorded), digestValue(generated))
	}
	if err := ValidateAtlasMissionDashboardClosureBinding(recorded); err != nil {
		t.Fatal(err)
	}
	if recorded.Status != "mission_dashboard_closure_bound" ||
		...
		recorded.ClaimsAuthorityAdvance ||
		!recorded.RSIRemainsDenied {
		t.Fatalf("v02 dashboard closure binding lost Mission closure guards: %#v", recorded)
	}
	rows := map[string]AtlasMissionDashboardClosureBindingRow{}
	for _, row := range recorded.Rows {
		rows[row.Repo] = row
	...
	if rows["ao-atlas"].ClosureEvidencePath != "docs/evidence/ao-atlas-feature-depth-wave-v02/nodes/mission-recommendation-feature-depth-next-wave-32/recommendation-readback-after.json" {
		t.Fatalf("ao-atlas row must bind source recommendation readback, got %s", rows["ao-atlas"].ClosureEvidencePath)
	}
