...
func TestFeatureDepthWaveMergeCheckBindingFixtureBindsMergeCommitsToPassedChecks(t *testing.T) {
	...
	for _, wave := range []string{"ao-atlas-feature-depth-wave-v01", "ao-atlas-feature-depth-wave-v02"} {
		t.Run(wave, func(t *testing.T) {
			nodeDir := filepath.Join(root, "docs", "evidence", wave, "nodes", "mission-recommendation-feature-depth-next-wave-12")
...
func TestFeatureDepthWaveMergeCheckBindingFixtureUsesTypedValidator(t *testing.T) {
	...
	for _, wave := range []string{"ao-atlas-feature-depth-wave-v01", "ao-atlas-feature-depth-wave-v02"} {
		t.Run(wave, func(t *testing.T) {
			path := filepath.Join(root, "docs", "evidence", wave, "nodes", "mission-recommendation-feature-depth-next-wave-12", "merge-check-binding.json")
