...
func validateRecommendationEvidenceTypedFile(path, schema string) (string, error) {
	...
	case AtlasEvidenceCatalogIndexExportContract:
		value, err := LoadJSON[AtlasEvidenceCatalogIndexExport](path)
		if err != nil {
	...
	case AtlasMonth3FinalReadinessReportContract:
		value, err := LoadJSON[AtlasMonth3FinalReadinessReport](path)
		if err != nil {
			return "typed:month3-final-readiness-report", err
		}
		return "typed:month3-final-readiness-report", ValidateAtlasMonth3FinalReadinessReport(value)
	...
	case AtlasMonth3ProviderModelProvenanceContract:
		value, err := LoadJSON[AtlasMonth3ProviderModelProvenance](path)
		if err != nil {
	...
	case AtlasMonth3ArchitectureSourceTruthChecklistContract:
		value, err := LoadJSON[AtlasMonth3ArchitectureSourceTruthChecklist](path)
		if err != nil {
	...
	case AtlasMonth3FoundrySafeNextWorkFixtureContract:
		value, err := LoadJSON[AtlasMonth3FoundrySafeNextWorkFixture](path)
		if err != nil {
	...
	case AtlasMonth6KillRestartGoldenPathRehearsalContract:
		value, err := LoadJSON[AtlasMonth6KillRestartGoldenPathRehearsal](path)
		if err != nil {
