...
func TestFeatureDepthWaveV02CompactionResumeRegressionPreservesReadyNodeAndExactAction(t *testing.T) {
	root := repoRoot(t)
	...
	if !strings.Contains(out.String(), "status=guarded") ||
		!strings.Contains(out.String(), "first_executable_node_before=mission-recommendation-feature-depth-next-wave-18") ||
		!strings.Contains(out.String(), "exact_next_action_preserved=true") {
		t.Fatalf("v02 compaction-resume-regression output missing regression state: %s", out.String())
	}
	recorded := mustLoadJSON[map[string]any](t, recordedPath)
	generated := mustLoadJSON[map[string]any](t, outPath)
	if digestValue(generated) != digestValue(recorded) {
		t.Fatalf("v02 compaction resume regression fixture changed\nwant %s\ngot  %s", digestValue(recorded), digestValue(generated))
	}
	promptBytes, err := os.ReadFile(sourcePromptMarkdown)
	if err != nil {
		t.Fatalf("read source prompt: %v", err)
	...
	for _, want := range []string{
		...
		if !strings.Contains(prompt, want) {
			t.Fatalf("source v02 compaction resume prompt missing %q:\n%s", want, prompt)
		}
	}
}
