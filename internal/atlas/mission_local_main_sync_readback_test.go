...
func TestFeatureDepthWaveLocalMainSyncReadbackFixtureValidatesNextNodeSelectionGate(t *testing.T) {
	root := repoRoot(t)
	nodeDir := filepath.Join(root, "docs", "evidence", "ao-atlas-feature-depth-wave-v01", "nodes", "mission-recommendation-feature-depth-next-wave-15")
...
func TestFeatureDepthWaveLocalMainSyncReadbackFixtureUsesTypedValidator(t *testing.T) {
	root := repoRoot(t)
	path := filepath.Join(root, "docs", "evidence", "ao-atlas-feature-depth-wave-v01", "nodes", "mission-recommendation-feature-depth-next-wave-15", "local-main-sync-readback.json")
