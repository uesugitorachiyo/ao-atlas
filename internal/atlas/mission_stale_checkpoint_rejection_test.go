...
func TestFeatureDepthWaveV02StaleCheckpointRejectionRejectsOutdatedContinuationPrompt(t *testing.T) {
	root := repoRoot(t)
	...
	if digestValue(fixture) != digestValue(recorded) {
		t.Fatalf("v02 stale checkpoint rejection fixture drifted\nwant %s\ngot  %s", digestValue(fixture), digestValue(recorded))
	}
	if fixture.Status != "rejected" ||
		fixture.RejectionReason != "stale_checkpoint" ||
		fixture.StaleCheckpoint.CompletedNodes != 1 ||
		...
		fixture.ClaimsAuthorityAdvance ||
		!fixture.RSIRemainsDenied {
		t.Fatalf("v02 fixture must reject stale continuation prompts without scheduling work or expanding authority: %#v", fixture)
	}
}
